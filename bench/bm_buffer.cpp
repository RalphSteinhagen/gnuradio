...
int main(int argc, char* argv[])
{
    ...
    app.add_option("--buffer_type",
                   ...
                   "buffer_host - POSIX, 5: portable, 6: malloc/C-Style)");
    app.add_flag(
    ...
    auto oneTest = [&](const int nP, const int nR) {
        using BufferType = buffer_host<int32_t, std::dynamic_extent, ProducerType:: Single>;
        ...
        case 0: {
            ...
            auto buffer = buffer_cpu_simple::make(sizeBuffer, sizeof(int32_t), bufferProperties);
            testDuration = testOldAPI(
                buffer, bufferProperties, veclen, samples, nP, nR, rt_prio);
        } break;
        case 1: {
            if (nP > 1) {
                fmt::print("nProducer {} unsupported for b-Type {}\n", nP, buffer_type);
            ...
            auto buffer = buffer_cpu_vmcirc::make(sizeBuffer, sizeof(int32_t), bufferProperties);
            testDuration = testOldAPI(
                buffer, bufferProperties, veclen, samples, nP, nR, rt_prio);
        } break;
        case 4: {
        ...
        case 6: {
            auto allocator = std::pmr::polymorphic_allocator<int32_t>(
                CStyleAllocator::defaultAllocator());
            Buffer auto buffer = BufferType(sizeBuffer, allocator);
            testDuration = testNewAPI(buffer, veclen, samples, nP, nR, rt_prio);
        } break;
        default: {
