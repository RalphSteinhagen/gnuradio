...
class buffer_skeleton
{
    ...
    public:
        ...
        {
            return 0;
        } // #items that can be read (may change/increase dynamically)

        // alternate: item-by-item interface -- do we need this
        template <bool strict_check = false>
        [[nodiscard]] constexpr U& operator[](const std::size_t index) const
            noexcept(!strict_check)
        {
            return T{};
        }
